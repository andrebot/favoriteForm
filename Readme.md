...
The test suite used was Karma+Jasmine. To run the tests you can use thi command (in case you installed karma-cli by executing `npm install-globals`):
...
application don't have the margins as it is shown in the wireframe because, once you trigger
...
so I decided not fixing it.

Jshint was configured to be as simple as possible, so that I had the minimal care for code
standard.

Gulp is the one responsible for creating the WebServer, I didn't implemented a Node server.

Did not minified the code or used gzip because I didn't had time to configured a production 
deploy task in gulp in the end, as I thought I would.

Did not used Babel to translate ES6 to ES5 because I was too lazy to do it, it would take 
